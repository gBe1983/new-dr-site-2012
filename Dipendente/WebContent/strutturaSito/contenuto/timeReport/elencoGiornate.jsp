...
<form action="./GestioneReport" method="post" name="listGiornate">
	...
	<input type="hidden" name="parametroCommessa" value="<%=id_commessa %>" id="parameter">
	...
	<%
		for(int x = 0; x < elencoGiorniCommessa.size(); x++){
			...
			if(giorno.getDescr_attivita().equals(nomeCommessa)){
				if(giorno.getId_planning() == parametro){
		...
		%>
						<tr>
							...
							<td style="background: #CCD;"><input type="checkbox" name="giorno<%=giorno.getId_planning() %>" value="<%=giorno.getId_planning() %>" disabled="disabled" checked="checked" class="case"></td>
						</tr>
		...
		%>				
						<tr>
							...
							<td style="background: #E7D6B6;"><input type="checkbox" name="giorno<%=giorno.getId_planning() %>" value="<%=giorno.getId_planning() %>" disabled="disabled" checked="checked" class="case"></td>
						</tr>			
		<%			
					}
				}else{
